...
public class CustomerController : MonoBehaviour, ITickUpdatable, IMinuteUpdatable
{
    ...
    private void ResetRuntimeData()
    {
        ...
        // 记录本次生成时间（用于 2 小时超时）
        spawnTime = TimeManager.Instance.GetComplexTime();
        HasTimedOut = false;
        _isQuiting = false;
        sr.DOKill(); // 清除可能残留的淡入淡出动画
    ...
    public void OnMinuteUpdate()
    {
        ...
        // 超时检测：存在超过 2 游戏小时 → 标记超时离开
        if (!HasTimedOut && !_isQuiting
            && TimeManager.Instance.TimeDistToNow(spawnTime) >= 60f)
        {
            HasTimedOut = true;
            customerMachine.ChangeState(new State_CustomerQuit(customerMachine, machineContext));
            return;
        }
    ...
    public bool OnEnter()
    {
        if (!_isQuiting && !DOTween.IsTweening(sr))
        {
            sr.DOKill();
            sr.DOFade(1f, 1.5f);
        }

        return sr.color.a >= 1f;
    }
    ...
    public bool OnQuit()
    {
        if (!_isQuiting)
        {
            _isQuiting = true;
            sr.DOKill();
            sr.DOFade(0f, 2f).SetEase(Ease.InQuad);
        }

        return sr.color.a <= 0.1f;
    }
